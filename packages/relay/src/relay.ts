...
const DEFAULT_OPTIONS: RelayCoreOptions = {
  pairTtlMs: 5 * 60_000,
  pairLimitPerMin: 30,
  pairMaxClients: 2,
  chanMaxMembers: 2,
...
export class RelayCore {
  ...
  private meta = new Map<Client, { nameplate?: string; token?: string }>();
  ...
  handleClose(client: Client): void {
    ...
    if (m?.token) {
      ...
      if (chan) chan.members = chan.members.filter((c) => c !== client);
    }
  ...
  private joinChan(client: Client, msg: { token?: unknown }): void {
    ...
    this.meta.set(client, { ...this.meta.get(client), token });
    client.send(JSON.stringify({ op: "chan-joined", peers: chan.members.length }));
    // 补发离线缓冲
    const now = Date.now();
    const fresh = chan.buffer.filter((e) => now - e.ts < this.opts.chanBufferTtlMs);
    chan.buffer = [];
  ...
  private chanData(client: Client, msg: { data?: unknown }): void {
    ...
    if (others.length === 0) {
      chan.buffer.push({ data: msg.data, ts: Date.now() });
